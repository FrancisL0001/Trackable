...
export function Brand({ size = "md" }: { size?: "md" | "lg" }) {
  const box = size === "lg" ? "w-10 h-10 rounded-[12px]" : "w-8 h-8 rounded-[9px]";
  const text = size === "lg" ? "text-2xl" : "text-xl";
  return (
    <div className={`flex items-center gap-2.5 font-extrabold ${text} tracking-tight`}>
      <span className={`grid place-items-center ${box} brand-mark text-white shadow-sm`}>
    ...
    <div className="mt-auto border-t border-border pt-3 flex flex-col gap-2">
      <div className="flex items-center gap-2.5 px-1.5 py-1.5 min-w-0">
        ...
        <div className="min-w-0 text-sm">
          ...
          <div className="text-content-faint truncate text-[0.78rem]">{user?.email}</div>
        </div>
      </div>
      <div className="flex gap-2">
        <ThemeToggle />
        <button className="btn btn-ghost btn-sm flex-1 justify-start" onClick={logout}>
